...
function MerchantHeaderRow({
	...
	return (
		...
		>
			...
			/>
			<button
				className="flex items-center gap-1.5 truncate text-left text-sm font-medium"
				onClick={onToggle}
			>
				<ChevronRightIcon
					className={cn(
...
function CategorizePage() {
	...
	const flatRows = useMemo<FlatRow[]>(() => {
		const filtered = txData.filter((tx) => {
			...
			if (filters.search && !tx.description.toUpperCase().includes(filters.search.toUpperCase()))
				return false
			return true
		})
								...
								>
									{row.kind === 'merchant-header' ? (
										<MerchantHeaderRow
											...
											isAllSelected={
												row.txIds.length > 0 &&
												row.txIds.every((id) => selectedIds.has(id))
											}
											isSomeSelected={
												!row.txIds.every((id) => selectedIds.has(id)) &&
												row.txIds.some((id) => selectedIds.has(id))
											}
