...
export const checkCmd = new Command("check")
  .description("Check for issues with deps/lint/types/format. If no options are provided, all checks are run.")
  ...
  .action(async ({ root, deps, lint, types, format, continue: useContinue }) => {
    ...
    let didErr = false;
    function getAndCheckOutcome({ exitCode, skippedCode }: { exitCode: number | null; skippedCode?: number }): string {
    ...
    if (deps) {
      ...
      table.push(["deps", checks.deps.join(" "), getAndCheckOutcome({ exitCode }), "Root"] satisfies TableRow);
    }
