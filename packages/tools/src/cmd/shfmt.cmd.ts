...
shfmtCmd
  ...
  .option("--skip-if-unavailable", "Only run if shfmt and ripgrep are available, otherwise skip with a warning.", false)
...
async function checkShfmtAvailable(skipIfUnavailable: boolean, mode: Mode): Promise<void> {
  const [shfmtExit, rgExit] = await Promise.all([which("shfmt", { nothrow: true }), which("rg", { nothrow: true })]);
