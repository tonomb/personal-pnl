...
function buildMonthlyPnL(month: string, rows: PnlRow[]): MonthlyPnL {
...
export const appRouter = router({
  ...
  pnl: router({
    getReport: publicProcedure.input(pnlGetReportInputSchema).query(async ({ input, ctx }) => {
    ...
    getMonth: publicProcedure.input(pnlGetMonthInputSchema).query(async ({ input, ctx }) => {
      const rows = await ctx.db
        ...
        .from(transactions)
        .leftJoin(categories, eq(transactions.categoryId, categories.id))
        .where(sql`strftime('%Y-%m', ${transactions.date}) = ${input.month}`)
        .groupBy(sql`strftime('%Y-%m', ${transactions.date})`, transactions.categoryId);

      return buildMonthlyPnL(input.month, rows);
    })
  })
});
